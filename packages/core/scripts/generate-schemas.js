...
async function generateSchemaForFile(inputPath, outputPath, currentFolder) {
  try {
    ...
    generatedContent = generatedContent.replace(/^\/\/ Generated by ts-to-zod\n/, generatedHeader);
...
async function processFolderParallel(folder) {
  ...
  const files = fs.readdirSync(typesDir).filter((f) => f.endsWith('.g.ts') && f !== 'index.ts');
...
async function main() {
  ...
  await Promise.all([processFolderParallel('requests'), processFolderParallel('responses')]);
  ...
  if (fs.existsSync(manualTypesDir)) {
    ...
    const files = fs.readdirSync(typesDir).filter((f) => f.endsWith('.ts') && f !== 'index.ts');
